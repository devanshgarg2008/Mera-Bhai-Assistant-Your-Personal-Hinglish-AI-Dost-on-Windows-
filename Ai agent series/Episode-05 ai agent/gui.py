...
class BhaiGUI(ctk.CTk):
    ...
    def start_listening(self):
        self.status_lbl.configure(text="Sun raha hoon... 👂", text_color="#00FF00")
    ...
    def speak_response(self, text):
        def play():
            try:
                # Pehle purana file hatao
                if os.path.exists("reply.mp3"):
                    os.remove("reply.mp3")
                
                # Nayi awaaz generate karo
                # Note: clean text for TTS (remove emojis or code symbols if needed)
                clean_text = text.replace('"', '').replace("'", "")
                os.system(f'edge-tts --voice hi-IN-MadhurNeural --text "{clean_text}" --write-media reply.mp3')
                
                # Play karo bina window khule
                playsound("reply.mp3")
